...
export default function ColecaoPage({ params, searchParams }: Props) {
  const { slug } = params;
  ...
  let products =
    slug === "lancamentos"
      ? productsData.filter((p) => p.category !== "outlet")
      : productsData.filter((p) => p.category === slug);

  // Filter by size
  if (searchParams.tamanho) {
    products = products.filter((p) => p.sizes.includes(searchParams.tamanho!));
  }

  // Sort
  if (searchParams.ordenar === "menor-preco") {
    products = [...products].sort((a, b) => a.price - b.price);
  } else if (searchParams.ordenar === "maior-preco") {
    products = [...products].sort((a, b) => b.price - a.price);
  } else if (searchParams.ordenar === "mais-vendidos") {
    // keep original order
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <section
        ...
        style={{
          ...
          background:
            slug === "outlet"
              ? "linear-gradient(135deg,#2e1a20,#8C2F39)"
              : "linear-gradient(160deg,#f5ece6,#e0cfc6)",
        }}
      >
        <h1
          className="text-3xl md:text-4xl font-extralight tracking-wider mb-2"
          style={{
            fontFamily: "serif",
            color: slug === "outlet" ? "#fff" : "#2e1a20",
          }}
        >
          {info.title}
        </h1>
        <p
          ...
          style={{ color: slug === "outlet" ? "rgba(255,255,255,0.8)" : "#666" }}
        >
          {info.description}
        </p>
        {slug === "outlet" && (
      ...
      {/* Filter + Sort bar */}
      <div className="border-b border-gray-100 bg-white sticky top-[70px] z-20">
        <div className="max-w-[1200px] mx-auto px-4 py-3 flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {searchParams.tamanho && (
              <a href="?" className="text-[11px] text-gray-400 hover:text-gray-700 ml-1">
                Limpar
              </a>
            )}
          </div>
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            {[
              ...
              <a
                ...
                style={{
                  color:
                    (searchParams.ordenar || "") === opt.value ? "#8C2F39" : "#666",
                  fontWeight: (searchParams.ordenar || "") === opt.value ? 700 : 400,
                }}
              >
                {opt.label}
              </a>
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-10">
        {products.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            ...
            <a href={`/colecao/${slug}`} className="text-[#8C2F39] text-sm underline">
              Limpar filtros
            </a>
          </div>
        ) : (
